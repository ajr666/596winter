...
# Context: In this part, you are going to play around with some commonly used pretrained text embeddings for text search. For example,
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type=50):
    ...
    (30 pts)
    """
    sum_embedding = np.zeros(int(model_type.split("d")[0]))
    ##################################
    ##### TODO: Add code here ########
    ##################################
    ...
    for w in words:
        word_embedding = get_glove_embeddings(w, word_index_dict, embeddings, model_type=model_type)
        sum_embedding += word_embedding

    avg_embedding = word_embedding / len(words)
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        for i, c in enumerate(categories):
            cat_embedding = get_glove_embeddings(c, word_index_dict, embeddings, model_type)

            x = torch.from_numpy(input_embedding).float()
            y = torch.from_numpy(cat_embedding).float()
            cos_val = cosine_similarity(x, y).item()
        ...
        for i, c in enumerate(categories):
            ...
            cat_embedding = category_embeddings[c]
    ...
    sorted_cosine = sorted(cosine_sim.items(), key=lambda x: x[1], reverse=True)
